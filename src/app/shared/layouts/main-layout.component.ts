...
import { MainLayoutState } from './main-layout-state';
...
export class MainLayoutComponent implements OnInit {
  layoutState = inject(MainLayoutState);
  ...
  ngOnInit() {
    ...
    this.router.events
      ...
      .subscribe(() => {
        this.layoutState.updateCurrentPage(this.router);
      });
  }
  ...
  onDocumentClick(event: Event) {
    ...
    if (!userDropdownContainer && this.isUserDropdownOpen()) {
      this.layoutState.closeUserDropdown();
    }
  }

  // Getters para el template
  getCurrentUser() {
    return this.layoutState.getCurrentUser();
  }

  getUserInitials() {
    return this.layoutState.userInitials();
  }

  getCurrentPageName() {
    return this.layoutState.getCurrentPageName();
  }

  getRoleClass(role: string | undefined) {
    return this.layoutState.getRoleClass(role);
  }

  // Getters para estados
  isSidebarOpen() {
    return this.layoutState.isSidebarOpen();
  }

  isDarkMode() {
    return this.layoutState.isDarkMode();
  }

  isUserDropdownOpen() {
    return this.layoutState.isUserDropdownOpen();
  }
  ...
  toggleDarkMode() {
    this.layoutState.toggleDarkMode();
  }
  ...
  toggleUserDropdown(event: Event) {
    ...
    this.layoutState.toggleUserDropdown();
  }
  ...
  logout() {
    this.layoutState.logout(this.authService, this.router);
  }

  hasRole(role: string): boolean {
    return this.layoutState.hasRole(role, this.authService);
  }

  hasAnyRole(roles: string[]): boolean {
    return this.layoutState.hasAnyRole(roles, this.authService);
  }
}
